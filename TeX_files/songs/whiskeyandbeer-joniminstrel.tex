\songtitle{Whiskey and Beer}{Joni Minstrel}{2006}

\guitarchord{C}
\guitarchord{F}
\guitarchord{G}

\begin{guitar}
	\songsection{Intro} {\footnotesize\begin{tabular}{|l|l|l|l|}
			C & C & F G & C
	\end{tabular}}

	\songsection{Verse 1}
	[C]Hop Master give us a [F]pint of [G]thy [C]best
	The [C]courage of our men will be [F]put to the [G]test
	Our [F]great revo[G]lution this [F]country will [G]save
	[C]We just need something to [F]help us [G]be [C]brave
	
	\begin{highlightbar}
		\songsection{Chorus}
		[C]Whiskey and Beer will [F]conquer our [G]fear.
		[C]Whiskey and Beer I [F]need it right [G]here.
		To [C]start a revolution takes [F]courage and [G]spunk.
		To [C]start a revolution it [F]helps to [G]be [C]drunk.
	\end{highlightbar}
	
	\songsection{Verse 2}
	We'll sing songs of bravery the best that you've heard
	Until we start slurring and forgetting the words
	When our bladders feel heavy but our heads feel light
	That's when we'll know that we're ready to fight
	
	\begin{highlightbar}
		\songsection{Chorus} \songsnippet{Whiskey and Beer will conquer our fear. ...}
	\end{highlightbar}
	
	\songsection{Verse 3}
	We'll get less coherent as the evening wears on
	Through blurry-eyed vision we'll watch for the dawn
	We'll stumble into battle our hearts will be stout
	And we'll be victorious unless$^1$  we pass out
	
	\begin{highlightbar}
		\songsection{Chorus} \songsnippet{Whiskey and Beer will conquer our fear. ...}
	\end{highlightbar}
	
	\songsection{Verse 4}
	We'll pick up our weapons and aim for their chests
	With balance unsteady we'll hope for the best
	And if we should fail, our lives to defend
	In one last act of courage we'll vomit on them
	
	\begin{highlightbar}
		\songsection{Chorus} \songsnippet{Whiskey and Beer will conquer our fear. ...}
	\end{highlightbar}
	\pagebreak
	\songsection{Verse 5}
	[C]Hop Master give us a [F]pint of [G]thy [C]best
	[C]One minstrel's courage will be [F]put to the [G]test
	The [F]pub is sur[G]rounded by [F]dawn I'll be [G]dead
	My [C]plan is to drink till I [F]can't feel [G]my [C]head
	
	\begin{highlightbar}
		\songsection{Chorus} \optionalChord{(x2)}
		[C]Whiskey and Beer will [F]conquer our [G]fear.
		[C]Whiskey and Beer I [F]need it right [G]here.
		To [C]start a revolution takes [F]courage and [G]spunk.
		To [C]start a revolution it [F]helps to [G]be [C]drunk.
	\end{highlightbar}
	
	\songsection{Outro}
	To [C]start a revolution it [F]heeelps [F]tooo [G]beee [C]drunk [G C]{}
\end{guitar}
\vfill
{\footnotesize \optionalChord{$^1$ text changed: (\textit{orig.}) "or else we pass out". Makes more sense like this because being drunk is an additional hindrance}}
\vspace{0.2em}

